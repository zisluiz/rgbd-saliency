...
cv::Mat FillGapCalc::getSLICOSegmentation(cv::Mat &rgb_8UC3)
{
...
std::vector<std::string> FillGapCalc::getExtFilesInDir ( std::string dir_path, std::string ext )
{
...
cv::Mat FillGapCalc::loadDisparity(std::string image_path)
{
...
cv::Mat FillGapCalc::loadDepth(std::string image_path)
{
...
void FillGapCalc::runDemo(std::string depth_dir, std::string rgb_dir, std::string output_dir, std::string ext)
{
...
std::vector<cv::Mat> FillGapCalc::fillListToImList(std::vector<std::vector<double> > fill_list, cv::Mat seg_32SC1)
{
...
std::vector<cv::Mat> FillGapCalc::fillListToImageList(std::vector<std::vector<double> > fill_list, cv::Mat seg_32SC1)
{
  std::vector<cv::Mat> im_list, merged_list;
  int n_partitions = fill_list.at(0).size();
  for (int i=0; i<n_partitions; i++)
...
// Compute LBE saliency maps example images.
void FillGapCalc::generateFillGapImages(std::string depth_dir, std::string rgb_dir, std::string fill_dir, std::string gap_dir, std::string ext, int bit_cnt)
{
  // get list of jpeg filenames in depth directory (assume same names for rgb)
  std::vector<std::string> im_files = getExtFilesInDir(depth_dir, ext);
  ...
  for (int i=0; i<im_files.size(); i++)
    {
      ...
      {
        cv::imwrite(fill_dir+"/"+im_files.at(i), results[j+k]);
        cv::imwrite(gap_dir+"/"+im_files.at(i), results[j+k+1]);
        k++;
      }
    }
}
